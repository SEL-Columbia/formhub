...
class Survey(Section):
    ...
    def load_elements_from_json(self, text_or_path):
        ...
        """
        try:
            # see if treating text_or_path as a path works
...
class SurveyBuilder(object):
    def __init__(self):
        # the following two dictionaries allow us to map types to classes
    ...
    def create_survey_element_from_dict(self, d):
        if d[u"type"] in self._question_classes:
            return self.create_question_from_dict(d)
        elif d[u"type"] in self._section_classes:
            elements = d.pop(u"elements")
            section_class = self._section_classes[d[u"type"]]
            result = section_class(**d)
            for element in elements:
                survey_element = create_survey_element_from_dict(element)
                result._add_element(survey_element)
            return result
        else:
            raise Exception("Unrecognized type", d[u"type"])
