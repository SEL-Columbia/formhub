...
def generate_pdf(id_string, user, permit_nums):
    ...
    pis = [x for x in all_instances 
             if x.to_dict().has_key(settings.PERMIT_NUM_FIELD) and 
                x.to_dict()[settings.PERMIT_NUM_FIELD] in permit_nums]
    ...
    obs_data = [ 
        {
          'permit_num': pi.to_dict()[settings.PERMIT_NUM_FIELD],
          'species': random.choice(['King Salmon', 'Chum Salmon']),
          'date': pi.to_dict().get(settings.DATE_FIELD, ''),
          'lat': pi.lat,
          'lng': pi.lng,
          'spawning': 2,
          'rearing': 3,
    ...
    minlng = min([x['lng'] for x in obs_data])
    minlat = min([x['lat'] for x in obs_data])
    maxlng = max([x['lng'] for x in obs_data])
    maxlat = max([x['lat'] for x in obs_data])
