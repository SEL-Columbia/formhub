...
var languageBasedLayers = {
  fr: {label: "Mapbox Streets (Français)", url: 'modilabs.map-vdpjhtgz'},
  es: {label: "Mapbox Streets (Español)", url: 'modilabs.map-5gjzjlah'}
};
...
var mapboxMaps = [
    {label: gettext('Mapbox Streets'), url: 'modilabs.map-iuetkf9u'},
    {label: gettext('MapBox Streets Light'), url: 'modilabs.map-p543gvbh'},
    {label: gettext('MapBox Streets Zenburn'), url: 'modilabs.map-bjhr55gf'},
    {label: gettext('Cloudless Earth'), url: 'modilabs.map-aef58tqo'}
];
...
var navContainerSelector = ".layer-viewbyButton";
...
var formResponseMngr = new FormResponseManager(mongoAPIUrl, loadResponseDataCallbackInitial);
var currentLanguageIdx = -1;
var customMapBoxTileLayer;
var legendsContainer;

var lastHash;
var action_button_container;
...
function initialize_map_embed() {
    // var map = L.mapbox.map('map_canvas', 'examples.map-y7l23tes').setView([37.9, -77], 5);

    // Make a new Leaflet map in your container div
    map = new L.mapbox.map(mapId).setView(centerLatLng, defaultZoom);
    ...
    map.on('draw:created', function (e) {
        ...
        if (type === 'marker') {
            // fire the modal
            addSubmissionAt(e.layer._latlng);
        }
    ...
    var tileJSONAddFn = function(mapData, addToMap) {
        ...
        var tilejson = mapData;
        return innerFn(tilejson);
    };
    var defaultMap;
    ...
    _.each(mapboxMaps, function(mapData, idx) {
        tileJSONAddFn(mapData, defaultMap == mapData);
    });
    ...
    action_button_container = $('<div class="action-button-container leaflet-control"></div>');
    $(".leaflet-top.leaflet-right").append(action_button_container);

    // info button
    var info_button = $('<div class="layer-infoButton"></div>');
    info_button.popover({title: mapTitle, content: mapDescription, html:true, placement: "left"});
    action_button_container.append(info_button);

    var refresh_button = $('<div class="layer-refreshButton refresh-map" title="'+gettext("Refresh Map")+'"></div>');
    refresh_button.click(function () { refreshMap(); });
    action_button_container.append(refresh_button);

    // download button
    var download_menu = $('<div class="layer-downloadButton"></div>');
    action_button_container.append(download_menu);

    dropDownContainer = $('<div class="dropdown pull-right"/>');
    dropdownCaretLink = $('<a href="#" class="dropdown-toggle" data-toggle="dropdown" />');
    dropdownCaretLink.attr('title', gettext("Data"));
    // dropdownCaretLink.html(gettext("Data"));
    dropDownContainer.append(dropdownCaretLink);

    var downloadUlContainer = $('<ul class="dropdown-menu download"/>');
    var csvLi = $('<li/>');
    var xlsLi = $('<li/>');
    var kmlLi = $('<li/>');

    var csvA = $('<a href="'+csv_url+'">'+gettext("csv")+'</a>');
    var xlsA = $('<a href="'+xls_url+'">'+gettext("xls")+'</a>');
    var kmlA = $('<a href="'+kml_url+'">'+gettext("kml")+'</a>');

    csvLi.append(csvA);
    xlsLi.append(xlsA);
    kmlLi.append(kmlA);

    downloadUlContainer.append(csvLi);
    downloadUlContainer.append(xlsLi);
    downloadUlContainer.append(kmlLi);
    dropDownContainer.append(downloadUlContainer);
    download_menu.append(dropDownContainer);
...
{
    console.log("loadFormJSONCallback");
    // we only want to load gps to begin with
    var fields = getBootstrapFields();
    var geoField = formJSONMngr.getGeoPointQuestion()[constants.NAME];

    // load responses
    formResponseMngr.loadResponseData({}, 0, null, geoField, fields, true);
}

function loadResponseSelectOneCallback() {
    console.log("loadResponseSelectOneCallback");
}

function loadResponseDataCallbackInitial()
{
    console.log("loadResponseDataCallbackInitial");
    formResponseMngr.callback = null;
    ...
    // set callback
    formResponseMngr.callback = loadResponseDataCallback;

    // relaunch
    var fields = getSelectOneFields();
    formResponseMngr.loadResponseData({}, 0, null, [], fields, true);
}

// callback called after response data has been loaded via the mongo form API
function loadResponseDataCallback()
{
    console.log("loadResponseDataCallback");
    ...
    // reload geojson so that select ones are inserted in point's properties
    geoJSON = formResponseMngr.getAsGeoJSON(true);
    _buildMarkerLayer(geoJSON);

    // create navigation holder
    var view_by_menu = $('<div class="layer-viewbyButton"></div>');
    action_button_container.append(view_by_menu);

    // just to make sure the nav container exists
    var navContainer = $(navContainerSelector);
    if(navContainer.length == 1)
    {
        currentLanguageIdx = 0;  // set it temporary for select ones to work

        // check if we have select one questions
        if(formJSONMngr.getNumSelectOneQuestions() > 0)
        {
            dropDownContainer = _createElementAndSetAttrs('div', {"class":"dropdown pull-right"});
            dropdownCaretLink = _createElementAndSetAttrs('a', {"href":"#", "class":"dropdown-toggle",
                "data-toggle":"dropdown", "title": gettext("View By")});
            ...
            view_by_menu.append(dropDownContainer);
        }
        ...
        if(formJSONMngr.supportedLanguages.length > 1)
        {
            ...
            var languages_menu = $('<div class="layer-languagesButton"></div>');
            action_button_container.append(languages_menu);

            dropDownContainer = _createElementAndSetAttrs('div', {"class":"dropdown language-picker pull-right"});
            dropdownCaretLink = _createElementAndSetAttrs('a', {"href":"#", "class":"dropdown-toggle",
                "data-toggle":"dropdown", "title": gettext("Language")});
            dropDownContainer.appendChild(dropdownCaretLink);
            ...
            languages_menu.append(dropDownContainer);
...
{
    console.log("_recolorMarkerLayer: " + questionName);
    var latLngArray = [];
...
{
    console.log("viewByChanged: "+ questionName);
